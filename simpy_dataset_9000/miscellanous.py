...
def unione_eventi1256():
    event_types = ["1a", "1b", "2", "5","6"]
    dfs = []
    for event_type in event_types:
        events = pd.read_csv(f"data/events{event_type}_sign.csv")
        events["evento"] = event_type
        if "1" in event_type:
            ...
            dfs.append(events.rename(columns={"A":"u1", "X":"u2", "Y":"u3"}))
        elif "5" in event_type:
            dfs.append(events.rename(columns={"A1":"u1", "X":"u2", "A2":"u3"}))
        elif "6" in event_type:
            dfs.append(events.rename(columns={"X1":"u1", "A":"u2", "X2":"u3"}))

    eventsevents = pd.concat(dfs)
    eventsevents.to_csv("data/events1256.csv", index=False)
    return
