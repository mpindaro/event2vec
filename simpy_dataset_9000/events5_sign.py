...
def group_e2():
    ...
    events2_df_grouped = eventi2.groupby(by=["timestamp", "A1", "X", "A2"], as_index=False).sum()
    eventi2 = events2_df_grouped


def get_part_a_ax(a, x):
    events_xy = eventi2[eventi2["A1"] == a]
...
def calcolo_z():
    ...
    for i, row in eventi_p.iterrows():
        ...
        n = partsAX[(row["A1"], row["X"])]["n_telefonate"]
