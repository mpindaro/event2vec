...
def group_e2():
    ...
    events2_df_grouped = eventi2.groupby(by=["timestamp", "X1", "A", "X2"], as_index=False).sum()
    eventi2 = events2_df_grouped


def get_part_a_ax(a, x):
    events_xy = eventi2[eventi2["X1"] == a]
    events_xy = events_xy[events_xy["A"] == x]
    intervals = []
    if len(events_xy) != 0:
        start = 0
        start_finestra = True
...
def get_p_cappuccio(a, x, y):
    events_ax = eventi2[eventi2["X1"] == a]
    events_ax = events_ax[events_ax["A"] == x]
    events_ax.reset_index(inplace=True, drop=True)
    events_axy = events_ax[events_ax["X2"] == y]
...
def calcolo_z():
    ...
    for i, row in eventi_p.iterrows():
        ...
        n = partsAX[(row["X1"], row["A"])]["n_telefonate"]
