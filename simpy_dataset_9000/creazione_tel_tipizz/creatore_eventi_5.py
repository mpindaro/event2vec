        ...
        if row["is_mittente_intercettato"] == "N" and row["is_destinatario_intercettato"] == "S":
            for i2 in range(i1 + 1, len(ifile)):
                ...
                if row2["timestamp"] > (timestamp1 + finestra):
                    break
                elif row2["mittente"] == x and row2["is_destinatario_intercettato"] == "N" and row["mittente"] == row2["destinatario"]:
                    y = row2["destinatario"]
...
if __name__ == "__main__":
    events2 = event2()
    events2df = pd.DataFrame(events2, columns=["timestamp", "A1", "X", "A2", "timestamp2", "esito_positivo"])
