...
def event2():
    ...
    for i1 in range(len(ifile)):
        ...
        if row["is_mittente_intercettato"] == "S" and row["is_destinatario_intercettato"] == "N":
            for i2 in range(i1 + 1, len(ifile)):

                row2 = ifile.iloc[i2]
                if row2["timestamp"] > (timestamp1 + finestra):
                    break
                elif row2["mittente"] == x and row2["is_destinatario_intercettato"] == "S" and row["mittente"] == row2["destinatario"]:
                    y = row2["destinatario"]
...
if __name__ == "__main__":
    events2 = event2()
    events2df = pd.DataFrame(events2, columns=["timestamp", "X1", "A", "X2", "timestamp2", "esito_positivo"])
